...
class MysqlQueryCountCheck < Sensu::Plugin::Check::CLI
  ...
  def run
    ...
    if length <= config[:crit]
      critical "Result count is below the CRITICAL limit: #{length} length / #{config[:crit]} limit"
    elsif length <= config[:warn]
      warning "Result count is below the WARNING limit: #{length} length / #{config[:warn]} limit"
    else
      ok 'Result count length is above thresholds'
    end
